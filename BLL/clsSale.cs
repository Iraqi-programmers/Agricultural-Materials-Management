...
namespace BLL
{
    public class clsSale : absClassesHelper
    {
        ...
        private clsSale(int id, int userId, DateTime date, int? personId = null) 
        { 
            Id = id;
            Date = date;
            PersonId = personId;
        ...
        public static async Task<clsSale?> FindAsync(int saleId)
        {
            ...
            return new clsSale(data?[0] as int? ?? 0, data?[3] as int? ?? 0, data?[1] as DateTime? ?? DateTime.MinValue, data?[2] as int? ?? 0) ?? null;
        }
        ...
        {
            var Id = await clsSalesData.AddSaleWithDetailsAsync(Details, userId, PersonId, fullName, nationalNum, phoneNum, address);
            return Id;
        }          

        public  async Task<bool> UpdateSaleDataAsync(int userId) => await clsSalesData.UpdateSaleDetailsAsync(userId, Details);
